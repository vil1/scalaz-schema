...
//schema: FSchema[Prim, SumTermId, ProductTermId, A],
  default: A
      ...
      SumTermId,
      ProductTermId,
      HEnvTK[Option, SchemaF[Prim, SumTermId, ProductTermId, ?[_], ?], ?[_], ?]
    ] {
  type HEnvSchema[F[_], X] = HEnvTK[Option, SchemaH, F, X]

  override val algebra = new (HEnvSchema[HSchema, ?] ~> HSchema) {

    def apply[X](env: HEnvSchema[HSchema, X]): HSchema[X] =
      env.ask.fold[HSchema[X]](Fix(env.fa))(x => Fix(ConstSchemaF(Fix(env.fa), x)))
  }
  override val coalgebra = new (HSchema ~> HEnvSchema[HSchema, ?]) {
    ...
    def apply[X](schema: HSchema[X]): HEnvSchema[HSchema, X] = schema.unFix match {
      ...
      case x: FieldF[HSchema, X, Prim, SumTermId, ProductTermId] =>
        if (x.id == name) HEnvTK(Some(default.asInstanceOf[X]), x) else HEnvTK(None, x)
...
trait HasMigrations[R <: Realisation] extends SchemaModule[R] {
  ...
  final class UpgradingSchema[G[_[_], _]: HFunctor](
  ...
  object Schema {
    ...
    def upgradingVia[G[_[_], _]: HFunctor](
      step: MigrationStep[R.Prim, R.SumTermId, R.ProductTermId, G]
    ): UpgradingSchema[G] =
